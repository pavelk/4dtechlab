...
=end
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      ...
      if options[:accessible]
        cal << %("><span class="hidden"> #{Date::MONTHNAMES[d.month]}</span></td>)
      else
        cal << %("></td>)
      end
    ...
    (last + 1).upto(beginning_of_week(last + 7, first_weekday) - 1)  do |d|
      ...
      cal << " weekendDay" if weekend?(d)
      if options[:accessible]
        cal << %("><span class='hidden'> #{Date::MONTHNAMES[d.mon]}</span></td>)
      else
        cal << %("></td>)        
      end
    ...
    if options[:previous_month_text] or options[:next_month_text]
      cal << %(<ul class="navi">)
      cal << %(<li class="fl">#{options[:previous_month_text]}</li>)
      cal << %(<li class="fr">#{options[:next_month_text]}</li>) if options[:next_month_text]
      cal << %(<li class="center">#{Date::MONTHNAMES[options[:month]]} #{options[:year]}</li>)
      cal << %(</ul>) 
    end  
